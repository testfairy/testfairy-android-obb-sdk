...
import static com.google.android.vending.expansion.downloader.impl.DownloaderService.DOWNLOAD_REQUIRED;
import static com.google.android.vending.expansion.downloader.impl.DownloaderService.LVL_CHECK_REQUIRED;
import static com.google.android.vending.expansion.downloader.impl.DownloaderService.NO_DOWNLOAD_REQUIRED;
import static com.google.android.vending.expansion.downloader.impl.DownloaderService.STATUS_SUCCESS;
...
public class TestFairyLVL implements Runnable, GetObbRequirementsRequestTask.OnResult {
	...
	public TestFairyLVL(Context context, DownloaderService downloaderService, String appToken) throws NoSuchFieldException, IllegalAccessException {
	...
	public static void registerService(DownloaderService downloaderService) {
		try {
			...
			TestFairyObbNotification.createNotificationChannel(downloaderService.getApplicationContext(), downloaderService);
	...
	public static void updateLVL(Context context, DownloaderService downloaderService, String appToken) {
		...
		try {
			h.post(new TestFairyLVL(c, downloaderService, appToken));
		} catch (Throwable t) {
			Log.e(Config.LOG_TAG, "Error setting up OBB with TestFairy, falling back to Google Play", t);
	...
	private static void setCustomNotificationReflection(Context context, DownloaderService downloaderService) throws NoSuchFieldException, IllegalAccessException {
		...
		TestFairyObbNotification testFairyObbNotification = new TestFairyObbNotification();
		...
		mCustomNotification.set(downloadNotification, testFairyObbNotification);

		Notification notification = testFairyObbNotification.updateNotification(context);
	...
	@Override
	public void onResult(TestFairyApkExpansionPolicy.OBBData[] result) {
	...
	private void downloadObbsIfNecessary(TestFairyApkExpansionPolicy.OBBData[] obbData) {
		try {
			...
			final APKExpansionPolicy aep = new TestFairyApkExpansionPolicy(context,
